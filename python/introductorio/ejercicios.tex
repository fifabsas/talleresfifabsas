...
\begin{figure}[H]
 \centering
 \includegraphics[width=0.15\textwidth]{../difusion/logo_python_fifa.png}
 \label{FIFA}
 \end{figure}
...
\url{http://goo.gl/EfXGFB}

Luego, para comenzar, en caso de que el IDE de las pc no funcionen, se puede probar programar online en la web: \url{https://cloud.sagemath.com/#settings}
...
\begin{enumerate}
 ...
 \item ¿Qué obtendrá de escribir $ 4==5 == 3=!2$? ¿Son necesarios los paréntesis?
 \item ¿Valen los operadores $=$ y $\ne$ para \textit{strings}?
\end{enumerate}
...
\begin{enumerate} 
 \item Arme una lista con las materias en las que debe el examen final.
 \item Pasó el cuatrimestre y nos colgamos todos. Agregue un elemento a la lista anterior (\textit{append}) y pedile que te muestre el segundo elemento.
...
Las librerías contienen funciones pensadas y distribuidas para extender las posibilidades de Python.
Lo primero que debemos hacer es cargar la librería con el comando \textit{import}.
Para resolver este ejercicio usaremos la librería \textit{math}.
...
Para encaminarnos al próximo encuentro, resulta básico y muy útil saber graficar funciones de una variable respecto a otra. Para esto hay funciones ya armadas en las librerías \textit{Numpy} y \textit{Matplotlib}.

Para experimentos numéricos, procederemos siempre de la misma manera:

\begin{enumerate}
 \item Crearemos un dominio homogéneo con \textit{numpy.linspace()}
 ...
 \item Graficaremos $x$ vs $y$ en un \textit{scatter} o en un \textit{plot}, aportando una grilla, dándole límites al dominio y la imagen mostrados, dándole nombres tanto a los ejes como al gráfico
\end{enumerate}

Interesante sería ver cómo hacer varios gráficos juntos (\textit{subplot}) o como hacer gráficos de tipo histograma, cosas que abarcaremos en el próximo encuentro.
